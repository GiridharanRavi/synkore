...
// HsnOption carries gst_percent + hsn_short_desc so other parts of the yarn
// item card (the GST badge in the card header, and the "HSN master GST
// rate" hint near the GST inputs) can keep working independently of the
// dropdown component itself.
// Backend /api/yarn-purchase-orders/meta/lookup must SELECT these columns:
//   SELECT id, hsn_code, hsn_short_desc, gst_percent, description
//   FROM hsn_codes WHERE status='Active' ORDER BY hsn_code
interface HsnOption {
  id: number;
  hsn_code: string;
  description?: string;
...
// Matches the page header pattern of other modules (e.g. Yarn Purchase
// Inward): a single "Export" button with a chevron that opens a small
// flyout offering CSV export, Excel export, and a print-friendly table view.
// Kept theme-agnostic (purple here) — same idea, different module colour.
...
function ExportMenu({ onExportCSV, onExportExcel, onPrint, disabled }: ExportMenuProps) {
...
// ─── GST Badge (matches HsnMaster colour scheme) ─────────────────────────────
// Colour-coded by GST slab: 0% green · ≤5% blue · ≤12% amber · ≤18% red · 28% pink
// Still used independently of the HSN dropdown — in the yarn item card
// header chip and the GST-rate hint box near the CGST/SGST/IGST inputs.
function GstBadge({ pct }: { pct?: string }) {
  ...
  return (
    <span style={{
      ...
      fontSize: 11, fontWeight: 700, background: bg, color, flexShrink: 0,
    }}>
      {isNaN(n) ? pct : `${n}%`}
    </span>
  );
...
// ─── HSN Dropdown ─────────────────────────────────────────────────────────────
// Restyled to match the Fabric Master page's HsnSearchDropdown format:
// a plain code-only trigger, a flat list of code + description rows (no
// column header bar, no per-row GST badge, no "HSN Master" footer label),
// themed in the Yarn PO module's purple instead of Fabric Master's teal.
// GST % is still surfaced elsewhere in the yarn item card (header chip +
// hint box) by looking the selected option up in hsnOptions directly, so
// nothing downstream needed to change except this component's own visuals
// and its onChange signature, which now just returns the selected id —
// the caller resolves hsn_code / gst_percent from that id itself.
interface HsnDropdownProps {
  value: string;                       // selected hsn_codes.id as string
...
function HsnDropdown({
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative', width: '100%' }}>
      {/* ── Trigger — code only, mirrors Fabric Master's plain pill ── */}
      <div
        onClick={() => { setOpen(o => !o); setTimeout(() => inputRef.current?.focus(), 50); }}
        style={{
          display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px',
          border: `1px solid ${open ? '#7c3aed' : '#cbd5e1'}`, borderRadius: 8,
          cursor: 'pointer', background: value ? '#f5f3ff' : '#fff', fontSize: 13,
          fontFamily: "'DM Sans', sans-serif", minHeight: 38,
          boxShadow: open ? '0 0 0 2px #ddd6fe' : 'none', transition: 'all 0.15s',
        }}
      >
        <span style={{
          flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          fontFamily: value ? "'DM Mono', monospace" : "'DM Sans', sans-serif",
          fontWeight: value ? 600 : 400, fontSize: value ? 12 : 13,
      ...
      {/* ── Dropdown — flat list, no header bar, no per-row GST badge ── */}
      {open && dropPos && (
        <div style={{
          position: 'fixed', top: dropPos.top, left: dropPos.left, width: Math.max(dropPos.width, 280),
          background: '#fff', border: '1px solid #ddd6fe', borderRadius: 10,
          zIndex: portalZIndex, boxShadow: '0 8px 32px rgba(0,0,0,0.18)',
          ...
          <div style={{ padding: '8px 10px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            <input
              ...
              style={{ flex: 1, border: 'none', outline: 'none', fontSize: 12, fontFamily: "'DM Sans', sans-serif", color: '#1e293b', background: 'transparent' }}
            />
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {filtered.length === 0 ? (
              ...
              <div
                key={h.id}
                onClick={() => { onChange(String(h.id)); setOpen(false); setQuery(''); }}
                style={{ padding: '8px 12px', cursor: 'pointer', borderBottom: '1px solid #f8fafc', background: String(h.id) === String(value) ? '#f5f3ff' : '#fff', transition: 'background 0.1s' }}
                onMouseEnter={e => (e.currentTarget.style.background = '#f5f3ff')}
                onMouseLeave={e => (e.currentTarget.style.background = String(h.id) === String(value) ? '#f5f3ff' : '#fff')}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 12, fontWeight: 700, color: '#6d28d9', background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: 5, padding: '1px 7px' }}>
                    {h.hsn_code}
                  </span>
                  {(h.hsn_short_desc || h.description) && (
...
// ─── Searchable Dropdown (for non-HSN fields) ─────────────────────────────────
...
function sanitizeItem(it: any): YarnPOItem {
  ...
  return {
    ...blank,
    _id:                it._id ?? blank._id,
    id:                 it.id,
    yarn_id:            safeStr(it.yarn_id),
    yarn_code:          safeStr(it.yarn_code),
    count_for_po:       safeStr(it.count_for_po),
    hsn_code:           safeStr(it.hsn_code ?? it.hsn_code_value),
    hsn_code_id:        safeStr(it.hsn_code_id),
    lot_no:             safeStr(it.lot_no),
    uom_id:             safeStr(it.uom_id),
    package_type:       safeStr(it.package_type),
    no_of_packages:     safeStr(it.no_of_packages),
    weight_per_package: safeStr(it.weight_per_package),
    total_weight:       safeStr(it.total_weight),
    cone_weight:        safeStr(it.cone_weight),
    no_of_cone_per_bag: safeStr(it.no_of_cone_per_bag),
    rate:               safeStr(it.rate),
    discount_type_id:   safeStr(it.discount_type_id),
    discount_pct:       safeStr(it.discount_pct),
    total_po_value:     safeStr(it.total_po_value),
    instructions:       safeStr(it.instructions),
    gst_pct:            safeStr(it.gst_pct),
    sgst_pct:           safeStr(it.sgst_pct),
    igst_pct:           safeStr(it.igst_pct),
    net_value:          safeStr(it.net_value),
  };
}

function sanitizeCoLink(l: any): CoLink {
  return {
    _id:           l._id ?? `co-${Date.now()}-${Math.random()}`,
    id:            l.id,
    co_id:         safeStr(l.co_id),
    co_no:         safeStr(l.co_no),
    customer_name: safeStr(l.customer_name),
    pwo_ids:       Array.isArray(l.pwo_ids) ? l.pwo_ids.map(String) : [],
    required_kgs:  safeStr(l.required_kgs),
  };
}
...
function sanitizeForm(data: any): YarnPO {
  return {
    id:                       data.id,
    rec_no:                   safeStr(data.rec_no),
    rec_date:                 toDateInput(data.rec_date),
    supplier_id:              safeStr(data.supplier_id),
    order_through:            safeStr(data.order_through) || 'Direct',
    agent_id:                 safeStr(data.agent_id),
    commission_pct:           safeStr(data.commission_pct),
    rate_type:                safeStr(data.rate_type) || 'Net rate',
    sup_address:              safeStr(data.sup_address),
    sup_pin_code:             safeStr(data.sup_pin_code),
    sup_district:             safeStr(data.sup_district),
    sup_state:                safeStr(data.sup_state),
    sup_country:              safeStr(data.sup_country),
    sup_gst_no:               safeStr(data.sup_gst_no),
    billing_same_as_supplier: safeStr(data.billing_same_as_supplier) || 'Yes',
    billing_supplier_id:      safeStr(data.billing_supplier_id),
    bill_address:             safeStr(data.bill_address),
    bill_pin_code:            safeStr(data.bill_pin_code),
    bill_district:            safeStr(data.bill_district),
    bill_state:               safeStr(data.bill_state),
    bill_country:             safeStr(data.bill_country),
    bill_gst_no:              safeStr(data.bill_gst_no),
    mill_same_as_supplier:    safeStr(data.mill_same_as_supplier) || 'Yes',
    mill_supplier_id:         safeStr(data.mill_supplier_id),
    mill_address:             safeStr(data.mill_address),
    mill_pin_code:            safeStr(data.mill_pin_code),
    mill_district:            safeStr(data.mill_district),
    mill_state:               safeStr(data.mill_state),
    mill_country:             safeStr(data.mill_country),
    mill_gst_no:              safeStr(data.mill_gst_no),
    company_address_id:       safeStr(data.company_address_id),
    comp_address:             safeStr(data.comp_address),
    comp_pin_code:            safeStr(data.comp_pin_code),
    comp_district:            safeStr(data.comp_district),
    comp_state:               safeStr(data.comp_state),
    comp_country:             safeStr(data.comp_country),
    comp_gst_no:              safeStr(data.comp_gst_no),
    exp_delivery:             toDateInput(data.exp_delivery),
    payment_term_id:          safeStr(data.payment_term_id),
    transport_freight_terms:  safeStr(data.transport_freight_terms) || 'Paid',
      ...
      {Array.from({ length: total }, (_, i) => (
        <button
          ...
          style={{
            ...
            height: 8,
            borderRadius: 4,
            background: active === i ? '#7c3aed' : '#ddd6fe',
            border: 'none',
            cursor: 'pointer',
            padding: 0,
...
// ─── Yarn Item Card ───────────────────────────────────────────────────────────
function YarnItemCard({
  item, idx, total, yarnOpts, hsnOptions, discOpts, onUpdate, onRemove,
}: {
  item: YarnPOItem; idx: number; total: number;
  yarnOpts: SDOption[];
  hsnOptions: HsnOption[];
  discOpts: SDOption[];
  onUpdate: (idx: number, updates: Partial<YarnPOItem>) => void;
  onRemove: (idx: number) => void;
}) {
  const inp = (field: keyof YarnPOItem) => ({
    value: item[field] as string,
    onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) =>
      onUpdate(idx, { [field]: e.target.value }),
    style: { ...s.input, width: '100%' },
  });

  // Resolve the full HSN option for header badge display
  ...
  return (
    <div style={{ background: '#fff', border: '1.5px solid #e2e8f0', borderRadius: 12, marginBottom: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', background: 'linear-gradient(90deg,#f8f5ff,#f0f9ff)', borderBottom: '1px solid #e2e8f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', minWidth: 0 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 13, color: '#1e293b' }}>
            {item.count_for_po || <span style={{ color: '#94a3b8', fontStyle: 'italic' }}>Yarn Line {idx + 1}</span>}
          </span>
          ...
          {selectedHsn ? (
            ...
            <span style={{ fontSize: 11, fontFamily: 'DM Mono,monospace', background: '#ede9fe', color: '#6d28d9', padding: '2px 8px', borderRadius: 20, fontWeight: 600 }}>
              HSN: {item.hsn_code}
            </span>
          ) : null}
        </div>
      ...
      <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        {/* Row 1: Count (Yarn), Count for PO, HSN Code */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>

          <Field label="Count (Yarn)" required type="lookup">
            <SearchableDropdown
              value={item.yarn_id}
              onChange={v => onUpdate(idx, { yarn_id: v })}
              options={yarnOpts}
              placeholder="Select actual count…"
              portalZIndex={10000}
            />
          </Field>
          ...
          <Field label="Count for PO" type="text">
            <input
              value={item.count_for_po}
              onChange={e => onUpdate(idx, { count_for_po: e.target.value })}
              placeholder="e.g. 62 / 2 NE"
              style={{ ...s.input, width: '100%' }}
            />
          </Field>

          {/* ── HSN Code field — Fabric Master style dropdown ── */}
            ...
            <HsnDropdown
              ...
              onChange={(id) => {
                ...
                onUpdate(idx, {
                  ...
                  // Pre-fill CGST/SGST from the HSN master's GST rate when not yet set
                  ...(id && !item.gst_pct ? {
                    gst_pct:  String(parseFloat(opt?.gst_percent ?? '0') / 2),
                    sgst_pct: String(parseFloat(opt?.gst_percent ?? '0') / 2),
                  } : {}),
                });
              }}
              hsnCodes={hsnOptions}
              ...
              portalZIndex={10000}
            />
            {/* Short description hint below the field */}
            {selectedHsn?.hsn_short_desc && (
        ...
        {/* Row 2: Lot No, UOM, Package Type */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          <Field label="Lot No" type="text">
            <input {...inp('lot_no')} placeholder="Optional" />
          </Field>

          <Field label="UOM" type="selection">
            <SearchableDropdown
              value={item.uom_id}
              onChange={v => onUpdate(idx, { uom_id: v })}
              options={HARDCODED_UOMS}
        ...
        {/* Row 3: Construction / Package Details */}
        <div style={{ borderTop: '1px dashed #e2e8f0', paddingTop: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12 }}>
            <Field label="No. of Pkgs" required type="number">
              <input type="number" {...inp('no_of_packages')} placeholder="0" min="0" />
            </Field>
            <Field label="Wt/Pkg (KGS)" type="number">
              <input type="number" {...inp('weight_per_package')} placeholder="0.000" min="0" step="0.001" />
            </Field>
            <Field label="Total Wt (KGS)" type="computed">
              <div style={{ padding: '8px 12px', background: '#fef9c3', border: '1px solid #fde68a', borderRadius: 8, fontSize: 13, fontWeight: 700, color: '#92400e', fontFamily: 'DM Mono,monospace', minHeight: 37, display: 'flex', alignItems: 'center' }}>
                {item.total_weight || '0.000'}
              </div>
            </Field>
            <Field label="Cone Wt (KGS)" type="number">
              <input type="number" {...inp('cone_weight')} placeholder="0.000" min="0" step="0.001" />
            </Field>
            <Field label="Cones/Bag" type="computed">
              <div style={{ padding: '8px 12px', background: '#fef9c3', border: '1px solid #fde68a', borderRadius: 8, fontSize: 13, fontWeight: 700, color: '#92400e', fontFamily: 'DM Mono,monospace', minHeight: 37, display: 'flex', alignItems: 'center' }}>
                {item.no_of_cone_per_bag || '—'}
              </div>
            </Field>
            <Field label="Rate (₹)" type="number">
              <input type="number" {...inp('rate')} placeholder="0.00" min="0" step="0.01" />
            </Field>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 12 }}>
          <Field label="Discount Type" type="lookup">
            <SearchableDropdown value={item.discount_type_id} onChange={v => onUpdate(idx, { discount_type_id: v })} options={discOpts} placeholder="Disc. Type…" portalZIndex={10000} />
          </Field>
          <Field label="Disc. %" type="number">
            <input type="number" {...inp('discount_pct')} placeholder="0.00" min="0" max="100" step="0.01" />
          </Field>
          <Field label="Basic Value" type="computed">
            <div style={{ padding: '8px 12px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, fontSize: 13, fontWeight: 700, color: '#c2410c', fontFamily: 'DM Mono,monospace', minHeight: 37, display: 'flex', alignItems: 'center' }}>
              ₹{item.total_po_value || '0.00'}
            </div>
          </Field>
        </div>

        {/* Row 5: Delivery Instruction + GST Panel */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 16, borderTop: '1px dashed #e2e8f0', paddingTop: 12 }}>
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder="AT SAME"
              style={{ ...s.input, resize: 'vertical', minHeight: 90, lineHeight: 1.6 }}
            />
          </div>
          ...
          <div style={{ minWidth: 320, background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '12px 16px' }}>
            ...
            {selectedHsn?.gst_percent && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10, padding: '6px 10px', background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: 8, fontSize: 11, color: '#6d28d9' }}>
                <Info size={12} />
                <span>HSN <strong>{selectedHsn.hsn_code}</strong> master GST rate: </span>
                <GstBadge pct={selectedHsn.gst_percent} />
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '8px 12px', alignItems: 'center', marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: '#374151', textAlign: 'right', fontFamily: 'DM Mono,monospace', minWidth: 72 }}>
                ₹{((parseFloat(item.total_po_value) || 0) * (parseFloat(item.gst_pct) || 0) / 100).toFixed(2)}
              </span>
              {/* SGST */}
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: '#374151', textAlign: 'right', fontFamily: 'DM Mono,monospace', minWidth: 72 }}>
                ₹{((parseFloat(item.total_po_value) || 0) * (parseFloat(item.sgst_pct) || 0) / 100).toFixed(2)}
              </span>
              {/* IGST */}
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: '#374151', textAlign: 'right', fontFamily: 'DM Mono,monospace', minWidth: 72 }}>
                ₹{((parseFloat(item.total_po_value) || 0) * (parseFloat(item.igst_pct) || 0) / 100).toFixed(2)}
              </span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1.5px solid #e2e8f0', paddingTop: 10 }}>
              ...
              <span style={{ fontSize: 16, fontWeight: 800, color: '#166534', fontFamily: 'DM Mono,monospace' }}>
                ₹{item.net_value || '0.00'}
              </span>
            </div>
          </div>
...
export default function YarnPurchaseOrderMaster() {
  ...
  const [lookup, setLookup]     = useState<LookupData>({
    suppliers: [], agents: [], yarns: [], uoms: [], discountTypes: [],
    paymentTerms: [], companyAddresses: [], customerOrders: [], pwos: [],
    hsnCodes: [],
  });
  ...
  const loadLookup = async () => {
    try {
      ...
      setLookup({ ...data, hsnCodes: data.hsnCodes ?? [] });
    } catch {
      pushToast('warning', 'Lookup Failed', 'Could not load master data.');
    }
  };
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      pushToast('error', 'Load Failed', 'Could not fetch purchase orders.');
    }
    setLoading(false);
  };
  ...
  const fillSupplier = (id: string, prefix: 'sup' | 'bill' | 'mill') => {
    ...
    if (!sup) return {};
    return {
      [`${prefix}_address`]:  safeStr(sup.address),
      [`${prefix}_pin_code`]: safeStr(sup.pin_code),
      [`${prefix}_district`]: safeStr(sup.district),
      [`${prefix}_state`]:    safeStr(sup.state),
      [`${prefix}_country`]:  safeStr(sup.country),
      [`${prefix}_gst_no`]:   safeStr(sup.gst_no),
    };
  };
  ...
  const fillCompanyAddress = (id: string) => {
    ...
    if (!ca) return {};
    return {
      comp_address:  safeStr(ca.address),
      comp_pin_code: safeStr(ca.pin_code),
      comp_district: safeStr(ca.district),
      comp_state:    safeStr(ca.state),
      comp_country:  safeStr(ca.country),
      comp_gst_no:   safeStr(ca.gst_no),
    };
  };
  ...
  const handleBillingSupplierChange = (id: string) => {
    setForm(f => ({ ...f, billing_supplier_id: id, ...fillSupplier(id, 'bill') }));
  };

  const handleMillSupplierChange = (id: string) => {
    setForm(f => ({ ...f, mill_supplier_id: id, ...fillSupplier(id, 'mill') }));
  };
  ...
  const updateItem = (idx: number, updates: Partial<YarnPOItem>) => {
    setForm(f => {
      ...
      if (updates.yarn_id !== undefined) {
        ...
        if (yarn) {
          merged.count_for_po = yarn.actual_count
            ? String(yarn.actual_count)
            : yarn.count ? String(yarn.count) : (yarn.yarn_code || yarn.short_name || '');
        } else {
          merged.count_for_po = '';
        }
  ...
  const updateCoLink = (idx: number, updates: Partial<CoLink>) => {
    setForm(f => {
      ...
      if (updates.co_id !== undefined) {
        ...
        merged.co_no         = safeStr(co?.co_no);
        merged.customer_name = safeStr(co?.customer_name);
        merged.pwo_ids       = [];
      }
      links[idx] = merged;
  ...
  const openEdit = async (id: number) => {
    try {
      ...
      pushToast('error', 'Load Failed', 'Could not load purchase order.');
    }
  };
  ...
  const validateTab = (tabIdx: number): string => {
    ...
    if (tabIdx === 1) {
      ...
      if (form.items.some(it => !it.no_of_packages || parseFloat(it.no_of_packages) <= 0))
        return 'All yarn lines need a valid No. of Packages.';
    }
    return '';
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        setOrders(p => p.map(o => o.id === editId ? saved : o));
      } else {
        setOrders(p => [saved, ...p].slice(0, pageSize));
        setTotal(p => p + 1);
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      pushToast('error', 'Delete Failed', 'Could not delete purchase order.');
    }
  };

  // ── Export / Print handlers ───────────────────────────────────────────────────
  // Operates on the currently-loaded page of `orders` (same rows visible in
  // the table). Columns mirror the table exactly so what you see is what
  // gets exported / printed.
  const exportRow = (o: YarnPO) => {
    ...
    return {
      'PO No':         o.rec_no ?? '',
      'Date':          toDateInput(o.rec_date) || '',
      'Supplier':      (o as any).supplier_name ?? '',
      'Order Through': o.order_through ?? '',
      'Rate Type':     o.rate_type ?? '',
      'PO Value':      poVal.toFixed(2),
      'Net Value':     netVal.toFixed(2),
  ...
  const handleExportExcel = () => {
    ...
    // No xlsx dependency required — Excel opens this HTML-table payload natively.
    // Swap in the SheetJS ('xlsx') library here if a true .xlsx binary is needed.
  ...
  const handlePrintTable = () => {
    ...
    win.document.write(`
      <html>
        <head>
          <title>Yarn Purchase Orders</title>
          <style>
  ...
  const compAddrOpts: SDOption[] = lookup.companyAddresses.map(c => ({ value: String(c.id), label: c.company_name, sub: c.address }));

  const yarnOpts: SDOption[] = lookup.yarns.map(y => ({
    value: String(y.id),
    label: yarnCountLabel(y),
    sub:   [y.yarn_code, y.count_type, y.category].filter(Boolean).join(' · ') || y.short_name,
  }));

  // ── hsnOptions passed as HsnOption[] directly into YarnItemCard ──────────────
  // Backend must return: id, hsn_code, hsn_short_desc, gst_percent, description
    ...
    <>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .ypo-add-btn { display:flex; align-items:center; gap:6px; background:#7c3aed; color:#fff; border:none; border-radius:9px; padding:9px 18px; font-size:13px; font-weight:700; cursor:pointer; font-family:'DM Sans',sans-serif; box-shadow:0 2px 8px rgba(124,58,237,0.35); white-space:nowrap; }
        .ypo-add-btn:hover { background:#6d28d9; }

        .ypo-export-btn { display:flex; align-items:center; gap:6px; background:#fff; color:#7c3aed; border:1.5px solid #ddd6fe; border-radius:9px; padding:9px 14px; font-size:13px; font-weight:700; cursor:pointer; font-family:'DM Sans',sans-serif; white-space:nowrap; }
        ...
        .ypo-export-item { display:flex; align-items:center; gap:10px; width:100%; padding:9px 14px; background:none; border:none; cursor:pointer; font-size:13px; color:#374151; font-family:'DM Sans',sans-serif; text-align:left; }
        ...
        .ypo-search { width:100%; padding:8px 12px 8px 34px; border:1px solid #cbd5e1; border-radius:8px; font-size:13px; font-family:'DM Sans',sans-serif; outline:none; }
        .ypo-search:focus { border-color:#7c3aed; }
        .ypo-filter-sel { border:1px solid #cbd5e1; border-radius:8px; padding:8px 10px; font-size:13px; font-family:'DM Sans',sans-serif; background:#fff; cursor:pointer; outline:none; }
        .ypo-page-size { display:flex; align-items:center; gap:6px; font-size:13px; color:#64748b; margin-left:auto; }
        .ypo-page-size select { border:1px solid #cbd5e1; border-radius:6px; padding:5px 8px; font-size:13px; font-family:'DM Sans',sans-serif; cursor:pointer; outline:none; }
        ...
        .ypo-table tbody tr:hover td { filter:brightness(0.97); }
        ...
        .ypo-action-group { display:flex; align-items:center; gap:5px; }
        .ypo-btn-edit { display:inline-flex; align-items:center; gap:3px; background:#faf5ff; color:#7c3aed; border:1px solid #c4b5fd; padding:4px 8px; border-radius:6px; font-size:11px; font-weight:700; cursor:pointer; }
        .ypo-btn-edit:hover { background:#ede9fe; }
        .ypo-btn-del  { display:inline-flex; align-items:center; gap:3px; background:#fff1f2; color:#dc2626; border:1px solid #fca5a5; padding:4px 8px; border-radius:6px; font-size:11px; font-weight:700; cursor:pointer; }
        .ypo-btn-del:hover { background:#fee2e2; }
        ...
        .ypo-modal { background:#fff; border-radius:14px; width:100%; max-width:960px; box-shadow:0 8px 40px rgba(0,0,0,0.22); display:flex; flex-direction:column; max-height:calc(100vh - 32px); }
        ...
        .ypo-tab { padding:12px 20px; font-size:13px; font-weight:600; cursor:pointer; border:none; background:none; color:#64748b; border-bottom:3px solid transparent; margin-bottom:-2px; white-space:nowrap; font-family:'DM Sans',sans-serif; transition:color .15s; }
        .ypo-tab.active { color:#7c3aed; border-bottom-color:#7c3aed; }
        .ypo-tab:hover:not(.active) { color:#475569; }
        ...
        .ypo-btn-cancel { padding:9px 16px; border:1px solid #cbd5e1; background:#fff; border-radius:8px; font-size:13px; font-weight:600; cursor:pointer; color:#475569; font-family:'DM Sans',sans-serif; }
        .ypo-btn-next   { display:flex; align-items:center; gap:6px; padding:9px 20px; border:none; background:#7c3aed; color:#fff; border-radius:9px; font-size:13px; font-weight:700; cursor:pointer; font-family:'DM Sans',sans-serif; box-shadow:0 2px 6px rgba(124,58,237,.3); }
        .ypo-btn-next:hover { background:#6d28d9; }
        .ypo-btn-prev   { display:flex; align-items:center; gap:5px; background:#f1f5f9; color:#475569; border:1px solid #cbd5e1; padding:9px 16px; border-radius:8px; font-size:13px; font-weight:600; cursor:pointer; font-family:'DM Sans',sans-serif; }
        .ypo-btn-prev:hover { background:#e2e8f0; }
        .ypo-btn-save   { display:flex; align-items:center; gap:6px; padding:9px 24px; border:none; background:#16a34a; color:#fff; border-radius:9px; font-size:13px; font-weight:700; cursor:pointer; font-family:'DM Sans',sans-serif; box-shadow:0 2px 6px rgba(22,163,74,.3); }
        ...
        .ypo-add-row-btn { display:flex; align-items:center; gap:5px; background:#faf5ff; color:#7c3aed; border:1px solid #ddd6fe; padding:7px 14px; border-radius:8px; font-size:12px; font-weight:700; cursor:pointer; font-family:'DM Sans',sans-serif; }
      ...
      <div className="ypo-wrap">
        ...
        <div className="ypo-page-header">
          <div>
            <h1>Yarn Purchase Order</h1>
            <p>{total} order{total !== 1 ? 's' : ''} recorded</p>
          </div>
        ...
        <div className="ypo-card">
          <div className="ypo-table-wrap">
            <table className="ypo-table">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={10} className="ypo-empty">
                    <Loader2 size={22} style={{ animation: 'spin 1s linear infinite', display: 'inline-block' }} />
                  </td></tr>
                ) : orders.length === 0 ? (
                  <tr><td colSpan={10} className="ypo-empty">
                    {search || filterSt ? 'No orders match your search.' : 'No purchase orders yet. Click "New Purchase Order" to create one.'}
                  </td></tr>
                ) : orders.map((o, i) => {
                  ...
                  return (
                    <tr key={o.id}>
                      ...
                      <td>
                        <div className="ypo-action-group">
                          <button className="ypo-btn-edit" onClick={() => openEdit(o.id!)}>✏️ {!isMobile && 'Edit'}</button>
                          <button className="ypo-btn-del"  onClick={() => handleDelete(o.id!)}>🗑 {!isMobile && 'Delete'}</button>
                        </div>
                      </td>
                    </tr>
                  );
            ...
            <div className="ypo-modal">
              ...
              <div className="ypo-modal-header">
                <div>
                  <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: '#fff' }}>
                    {editId ? '✏️ Edit Purchase Order' : '➕ New Yarn Purchase Order'}
                  </h2>
                  {editId && form.rec_no && (
                    <span style={{ fontSize: 11, color: '#ddd6fe', fontFamily: 'DM Mono,monospace' }}>{form.rec_no}</span>
                  )}
                </div>
                <button style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center' }} onClick={() => setShowForm(false)}>
                  <X size={22} color="#fff" />
                </button>
              </div>
              ...
              <div className="ypo-tabs">
                {TAB_LABELS.map((tab, i) => (
                  <button key={i} className={`ypo-tab${activeTab === i ? ' active' : ''}`} onClick={() => setActiveTab(i)}>{tab}</button>
                ))}
              </div>
                ...
                {activeTab === 0 && (
                  <div className="ypo-tab-body">
                    <div className="ypo-grid" style={{ marginBottom: 4 }}>
                      <Field label="Rec No" type="text">
                        <input value={form.rec_no ?? 'Auto-generated'} disabled style={{ ...s.input, ...s.inputDisabled }} />
                      </Field>
                      <Field label="Rec Date" required type="date">
                        <input type="date" value={toDateInput(form.rec_date)} onChange={e => set('rec_date', e.target.value)} style={s.input} />
                      </Field>
                      ...
                      <div style={{ marginTop: 12 }}>
                        ...
                        <div className="ypo-grid" style={{ marginTop: 14 }}>
                          <Field label="Order Through" type="selection">
                            <select value={form.order_through} onChange={e => set('order_through', e.target.value)} style={s.input}>
                        ...
                        <div className="ypo-grid" style={{ marginTop: 14 }}>
                          <Field label="Agent" type="lookup">
                            <SearchableDropdown
                              ...
                              onChange={v => {
                                set('agent_id', v);
                                const a = lookup.agents.find(ag => String(ag.id) === v);
                                if (a?.commission_pct) set('commission_pct', safeStr(a.commission_pct));
                              }}
                              options={agentOpts} placeholder="Select agent… (if applicable)" portalZIndex={9500}
                            />
                          </Field>
                          <Field label="Commission %" type="number">
                            <input type="number" value={form.commission_pct} onChange={e => set('commission_pct', e.target.value)} style={s.input} placeholder="0.00" step="0.01" min="0" max="100" />
                          </Field>
                        </div>
                        {form.agent_id && (
                          <div className="ypo-info-box" style={{ marginTop: 8 }}>
                            <Info size={14} style={{ flexShrink: 0, marginTop: 1 }} />
                            <span>
                              Agent: <strong>{lookup.agents.find(a => String(a.id) === form.agent_id)?.agent_name}</strong>
                              {form.commission_pct && <> · Commission: <strong>{form.commission_pct}%</strong></>}
                            </span>
                          </div>
                        )}
                        {form.supplier_id && (
                    ...
                    {sec.billing && (
                      <div style={{ marginTop: 12 }}>
                        <div className="ypo-same-bar">
                          <label className="ypo-toggle">
                            <input type="checkbox" checked={form.billing_same_as_supplier === 'Yes'} onChange={e => handleBillingSameChange(e.target.checked ? 'Yes' : 'No')} />
                            <span className="ypo-toggle-slider" />
                          </label>
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 700, color: '#374151' }}>Supplier Billing Name same as Supplier Name</div>
                            <div style={{ fontSize: 11, color: '#64748b' }}>Toggle off to select a different billing entity</div>
                          </div>
                          <span style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 700, padding: '3px 9px', borderRadius: 20, background: form.billing_same_as_supplier === 'Yes' ? '#dcfce7' : '#f3f4f6', color: form.billing_same_as_supplier === 'Yes' ? '#166534' : '#374151', border: '1px solid', borderColor: form.billing_same_as_supplier === 'Yes' ? '#86efac' : '#cbd5e1' }}>
                            {form.billing_same_as_supplier === 'Yes' ? '✓ Same' : 'Different'}
                          </span>
                        </div>
                        {form.billing_same_as_supplier === 'No' && (
                          <div style={{ marginTop: 10 }}>
                            <Field label="Supplier Billing Name" type="lookup">
                              <SearchableDropdown value={form.billing_supplier_id} onChange={handleBillingSupplierChange} options={supOpts} placeholder="Select supplier for billing…" portalZIndex={9500} />
                            </Field>
                          </div>
                        )}
                        {(form.billing_same_as_supplier === 'Yes' || form.billing_supplier_id) && (
                          <div className="ypo-grid" style={{ marginTop: 10 }}>
                            <Field label="Address"  type="autofill"><input value={form.bill_address}  disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="District" type="autofill"><input value={form.bill_district} disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="State"    type="autofill"><input value={form.bill_state}    disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="Pin Code" type="autofill"><input value={form.bill_pin_code} disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="Country"  type="autofill"><input value={form.bill_country}  disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="GST No"   type="autofill"><input value={form.bill_gst_no}   disabled style={{ ...s.input, ...s.inputDisabled, fontFamily: 'DM Mono,monospace' }} /></Field>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {sec.mill && (
                      <div style={{ marginTop: 12 }}>
                        <div className="ypo-same-bar">
                          <label className="ypo-toggle">
                            <input type="checkbox" checked={form.mill_same_as_supplier === 'Yes'} onChange={e => handleMillSameChange(e.target.checked ? 'Yes' : 'No')} />
                            <span className="ypo-toggle-slider" />
                          </label>
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 700, color: '#374151' }}>Is Mill Name same as Supplier Name?</div>
                            <div style={{ fontSize: 11, color: '#64748b' }}>Toggle off to select a different mill</div>
                          </div>
                          <span style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 700, padding: '3px 9px', borderRadius: 20, background: form.mill_same_as_supplier === 'Yes' ? '#dcfce7' : '#f3f4f6', color: form.mill_same_as_supplier === 'Yes' ? '#166534' : '#374151', border: '1px solid', borderColor: form.mill_same_as_supplier === 'Yes' ? '#86efac' : '#cbd5e1' }}>
                            {form.mill_same_as_supplier === 'Yes' ? '✓ Same' : 'Different'}
                          </span>
                        </div>
                        {form.mill_same_as_supplier === 'No' && (
                          <div style={{ marginTop: 10 }}>
                            <Field label="Mill Name" type="lookup">
                              <SearchableDropdown value={form.mill_supplier_id} onChange={handleMillSupplierChange} options={supOpts} placeholder="Select mill supplier…" portalZIndex={9500} />
                            </Field>
                          </div>
                        )}
                        {form.mill_same_as_supplier === 'Yes' && form.supplier_id && (
                          <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#64748b' }}>
                            <span style={{ fontSize: 11, background: '#dcfce7', color: '#166534', padding: '2px 8px', borderRadius: 12, fontWeight: 700, border: '1px solid #86efac' }}>
                              Mill: {lookup.suppliers.find(s => String(s.id) === form.supplier_id)?.supplier_name ?? '—'}
                            </span>
                          </div>
                        )}
                        {(form.mill_same_as_supplier === 'Yes' || form.mill_supplier_id) && (
                          <div className="ypo-grid" style={{ marginTop: 10 }}>
                            <Field label="Address"  type="autofill"><input value={form.mill_address}  disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="District" type="autofill"><input value={form.mill_district} disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="State"    type="autofill"><input value={form.mill_state}    disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="Pin Code" type="autofill"><input value={form.mill_pin_code} disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="Country"  type="autofill"><input value={form.mill_country}  disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="GST No"   type="autofill"><input value={form.mill_gst_no}   disabled style={{ ...s.input, ...s.inputDisabled, fontFamily: 'DM Mono,monospace' }} /></Field>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    <SectionHead title="Company Billing Address" open={sec.company} onToggle={() => toggle('company')} />
                    {sec.company && (
                      <div style={{ marginTop: 12 }}>
                        <Field label="Company Billing Address (Use Company)" required type="lookup">
                          <SearchableDropdown
                            value={form.company_address_id}
                            onChange={v => {
                              const fills = fillCompanyAddress(v);
                              setForm(f => ({ ...f, company_address_id: v, ...fills }));
                            }}
                            options={compAddrOpts} placeholder="Select company…" portalZIndex={9500}
                          />
                        </Field>
                        <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 4 }}>
                          Address of the VP company to be mentioned by the supplier in their bill.
                        </div>
                        {form.company_address_id ? (
                          <div className="ypo-grid" style={{ marginTop: 10 }}>
                            <Field label="Address"  type="autofill"><input value={form.comp_address}  disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="District" type="autofill"><input value={form.comp_district} disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="State"    type="autofill"><input value={form.comp_state}    disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="Pin Code" type="autofill"><input value={form.comp_pin_code} disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="Country"  type="autofill"><input value={form.comp_country}  disabled style={{ ...s.input, ...s.inputDisabled }} /></Field>
                            <Field label="GST No"   type="autofill"><input value={form.comp_gst_no}   disabled style={{ ...s.input, ...s.inputDisabled, fontFamily: 'DM Mono,monospace' }} /></Field>
                          </div>
                        ) : (
                          <div className="ypo-info-box" style={{ marginTop: 8 }}>
                            <Info size={14} style={{ flexShrink: 0, marginTop: 1 }} />
                            <span>Select a company above to autofill its billing address and GST details.</span>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {sec.delivery && (
                      <div className="ypo-grid" style={{ marginTop: 12 }}>
                        <Field label="Exp. Delivery" type="date">
                          <input type="date" value={toDateInput(form.exp_delivery)} onChange={e => set('exp_delivery', e.target.value)} style={s.input} />
                        </Field>
                        <Field label="Payment Terms" type="lookup">
                          <SearchableDropdown value={form.payment_term_id} onChange={v => set('payment_term_id', v)} options={payOpts} placeholder="Select payment terms…" portalZIndex={9500} />
                        </Field>
                        <Field label="Transport / Freight Terms" type="selection">
                          <select value={form.transport_freight_terms} onChange={e => set('transport_freight_terms', e.target.value)} style={s.input}>
                            <option>Paid</option>
                            <option>Ex-mill</option>
                          </select>
                        </Field>
                ...
                {activeTab === 1 && (
                  <div className="ypo-tab-body">
                    ...
                    <div className="ypo-grand-total">
                      <div className="ypo-grand-total-item">
                        <span>Grand PO Value</span>
                        <strong style={{ color: '#c2410c' }}>₹ {grandPoValue.toFixed(2)}</strong>
                      </div>
                      <div style={{ width: 1, height: 32, background: '#e2e8f0' }} />
                      <div className="ypo-grand-total-item">
                        <span>Grand Net Value (incl. Tax)</span>
                        <strong style={{ color: '#166534' }}>₹ {grandNetValue.toFixed(2)}</strong>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="ypo-tab-body">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: '#64748b', marginTop: 3 }}>
                          Link this PO to open Customer Orders and optionally select associated PWOs.
                        </div>
                      </div>
                    ...
                    {form.co_links.length === 0 ? (
                      <div style={{ textAlign: 'center', padding: '32px 16px', color: '#94a3b8', fontSize: 13, border: '2px dashed #e2e8f0', borderRadius: 12, background: '#fafbfc' }}>
                        ...
                        <div style={{ marginTop: 12, fontSize: 12, color: '#a3b8cc' }}>This step is optional — you can save the PO without linking a CO.</div>
                      </div>
                    ) : (
                      <table className="ypo-co-table">
                        <thead>
                          <tr>
                            ...
                            <th style={{ minWidth: 220 }}>Customer Order No <FTypeBadge type="lookup" /></th>
                            <th style={{ minWidth: 240 }}>Associated PWOs <FTypeBadge type="selection" /></th>
                            <th style={{ minWidth: 130 }}>Required KGS <FTypeBadge type="number" /></th>
                            <th style={{ width: 40 }}></th>
                          </tr>
                        </thead>
                        <tbody>
                          {form.co_links.map((link, idx) => {
                            const linkedPwos = lookup.pwos.filter(p =>
                              (link.co_id && String(p.co_id) === link.co_id) ||
                              (link.co_no && p.co_no === link.co_no)
                            );
                            return (
                              <tr key={link._id}>
                                ...
                                <td style={{ overflow: 'visible' }}>
                                  ...
                                  {link.customer_name && (
                                    <div style={{ fontSize: 11, color: '#64748b', marginTop: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
                                      <span style={{ color: '#7c3aed' }}>👤</span> {link.customer_name}
                                    </div>
                                  )}
                                </td>
                                <td>
                                  {!link.co_id ? (
                                    <span style={{ fontSize: 12, color: '#94a3b8', fontStyle: 'italic' }}>— Select Customer Order first —</span>
                                  ) : linkedPwos.length === 0 ? (
                                    <span style={{ fontSize: 12, color: '#f59e0b', display: 'flex', alignItems: 'center', gap: 4 }}>
                                      <AlertTriangle size={12} /> No PWOs found for this CO
                                    </span>
                                  ) : (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                                      {linkedPwos.map(pwo => (
                                        <label key={pwo.id} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, cursor: 'pointer', padding: '4px 6px', borderRadius: 6, background: link.pwo_ids.includes(String(pwo.id)) ? '#ede9fe' : '#f8fafc', border: `1px solid ${link.pwo_ids.includes(String(pwo.id)) ? '#c4b5fd' : '#e2e8f0'}` }}>
                                          <input
                                            type="checkbox"
                                            checked={link.pwo_ids.includes(String(pwo.id))}
                                            onChange={e => {
                                              const ids = e.target.checked
                                                ? [...link.pwo_ids, String(pwo.id)]
                                                : link.pwo_ids.filter(id => id !== String(pwo.id));
                                              updateCoLink(idx, { pwo_ids: ids });
                                            }}
                                          />
                                          <span style={{ fontFamily: 'DM Mono,monospace', color: '#1d4ed8', fontWeight: 600 }}>{pwo.wo_no}</span>
                                          <span style={{ fontSize: 10, padding: '1px 7px', borderRadius: 12, background: '#fef3c7', color: '#92400e', fontWeight: 700 }}>{pwo.status}</span>
                                        </label>
                                      ))}
                                      {link.pwo_ids.length > 0 && (
                                        <div style={{ fontSize: 10, color: '#7c3aed', marginTop: 2 }}>✓ {link.pwo_ids.length} PWO{link.pwo_ids.length > 1 ? 's' : ''} selected</div>
                                      )}
                                    </div>
                                  )}
                                </td>
                                <td>
                                  <input type="number" value={link.required_kgs} onChange={e => updateCoLink(idx, { required_kgs: e.target.value })} style={{ ...s.input, minWidth: 100 }} placeholder="KGS (optional)" min="0" step="0.001" />
                                </td>
                                <td style={{ textAlign: 'center' }}>
                                  <button onClick={() => removeCoLink(idx)} style={{ background: '#fff1f2', color: '#ef4444', border: '1px solid #fca5a5', width: 28, height: 28, borderRadius: 7, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <X size={13} />
                                  </button>
                                </td>
                              </tr>
                    ...
                    {form.co_links.length > 0 && (
                      <div style={{ marginTop: 14, padding: '10px 14px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, fontSize: 12, color: '#64748b', display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                        <span>🔗 <strong style={{ color: '#1e293b' }}>{form.co_links.length}</strong> CO{form.co_links.length > 1 ? 's' : ''} linked</span>
                        <span>📦 <strong style={{ color: '#1e293b' }}>{form.co_links.reduce((s, l) => s + l.pwo_ids.length, 0)}</strong> PWO{form.co_links.reduce((s, l) => s + l.pwo_ids.length, 0) !== 1 ? 's' : ''} selected</span>
                        {form.co_links.some(l => l.required_kgs) && (
                          <span>⚖️ Total Required: <strong style={{ color: '#1e293b' }}>{form.co_links.reduce((s, l) => s + (parseFloat(l.required_kgs) || 0), 0).toFixed(3)} KGS</strong></span>
                        )}
                      </div>
                    )}

                    {/* Review summary */}
                    <div style={{ marginTop: 20, padding: '14px 16px', background: '#faf5ff', border: '1.5px solid #ddd6fe', borderRadius: 12 }}>
                      <div style={{ fontSize: 12, fontWeight: 800, color: '#6d28d9', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>
                        📋 Review Summary before saving
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
                        {[
                          { label: 'Supplier',      value: lookup.suppliers.find(s => String(s.id) === form.supplier_id)?.supplier_name || '—' },
                          { label: 'Rec Date',      value: toDateInput(form.rec_date) || '—' },
                          { label: 'Status',        value: form.status },
                          { label: 'Order Through', value: form.order_through },
                          { label: 'Agent',         value: lookup.agents.find(a => String(a.id) === form.agent_id)?.agent_name || '—' },
                          { label: 'Commission %',  value: form.commission_pct || '—' },
                          { label: 'Yarn Lines',    value: `${form.items.length} line(s)` },
                          { label: 'Grand PO Val',  value: `₹ ${grandPoValue.toFixed(2)}` },
                          { label: 'Grand Net Val', value: `₹ ${grandNetValue.toFixed(2)}` },
                          { label: 'COs Linked',    value: `${form.co_links.length}` },
                          { label: 'Payment Terms', value: lookup.paymentTerms.find(p => String(p.id) === form.payment_term_id)?.payment_term_name || '—' },
                        ].map(row => (
                          <div key={row.label} style={{ background: '#fff', border: '1px solid #ede9fe', borderRadius: 8, padding: '8px 12px' }}>
              ...
              <div className="ypo-modal-footer">
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {activeTab > 0 && (
                    <button className="ypo-btn-prev" onClick={() => { setError(''); setActiveTab(t => t - 1); }}>
                      ← Prev
                    </button>
                  )}
                  {!isLastTab && (
                    <button className="ypo-btn-next" onClick={handleNext}>Next →</button>
                  )}
                  {isLastTab && (
                    <button className="ypo-btn-save" onClick={handleSave} disabled={saving}>
                      {saving
                        ? <><Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} /> Saving…</>
                        : editId ? '✏️ Update PO' : '💾 Save PO'
                      }
                    </button>
                  )}
                </div>
