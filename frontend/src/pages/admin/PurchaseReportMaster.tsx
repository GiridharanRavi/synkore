// frontend/src/pages/admin/PurchaseReportMaster.tsx
//
// Purchase Report — the mirror image of Sales Report, built on the same
// data Account Details' Payment Out tab uses (Purchase Invoices [Fabric +
// Yarn] = Debit, Payments Out = Credit), so the two pages can never
// disagree.
//
// Columns: Purchase Date · Purchase Invoice No · Supplier Name · Bill To ·
//          Credit · Debit · Balance (running, resets per supplier)
//
// Chart: identical pseudo-3D isometric bar chart to Sales Report (monthly
// Debit vs Credit, with a cumulative Balance line), rendered below the
// table. Same hand-built three-shapes-per-bar technique since Recharts has
// no native 3D chart type.
//
// Export menu: CSV / Excel / Print / PDF (PDF renders the chart to an
// image via html2canvas and embeds it above the table, same as Sales
// Report).
//
// ─────────────────────────────────────────────────────────────────────────
...
//
// REQUIRES: frontend/src/api/purchaseReportServices.ts to exist at that
// path (adjust the import below if you merged those exports into a shared
// services.ts instead).
// ─────────────────────────────────────────────────────────────────────────
...
import {
  ...
  ArrowDownToLine, ArrowUpFromLine, Wallet, Truck, Calendar,
  BarChart3,
} from 'lucide-react';
...
import {
  fetchPurchaseLedger,
  fetchPurchaseLedgerAll,
  fetchPurchaseReportSummary,
  fetchPurchaseReportTrend,
  fetchPurchaseReportSuppliers,
  ...
  return (
    <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 9999, display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 360, width: 'calc(100vw - 40px)', pointerEvents: 'none' }}>
      {toasts.map(t => {
        const c = cfg[t.type];
        return (
          <div key={t.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, background: c.bg, border: `1px solid ${c.border}`, borderRadius: 10, padding: '12px 14px', boxShadow: '0 4px 16px rgba(0,0,0,0.12)', pointerEvents: 'all', animation: 'prToastIn 0.25s ease-out', fontFamily: "'DM Sans', sans-serif" }}>
...
// Loosely-typed on purpose — see SalesReportMaster.tsx for the rationale
// (Recharts' Tooltip formatter props don't structurally match a strict
// (n: number) => string signature).
...
// ── Pseudo-3D bar shape (identical technique to Sales Report) ─────────────
...
function ExportMenu({
  ...
  summary: PurchaseReportSummary | null;
  filtersLabel: string;
}) {
  ...
  // image, and the full data table (via jspdf-autotable). Same layout as
  // Sales Report's PDF export.
  const exportPdf = async () => {
    ...
    try {
      ...
      doc.text('Purchase Report', margin, 40);
      doc.setFontSize(10);
      doc.setTextColor(100, 116, 139);
      doc.text(filtersLabel || 'All suppliers · All dates', margin, 58);
      doc.text(new Date().toLocaleString('en-IN'), pageWidth - margin, 40, { align: 'right' });

      if (summary) {
        doc.setFontSize(10);
        doc.setTextColor(30, 41, 59);
        const line = `Total Debit: ${inr(summary.total_debit)}    Total Credit: ${inr(summary.total_credit)}    Net Balance: ${inr(summary.net_balance)}    Invoices: ${summary.invoice_count}    Suppliers: ${summary.supplier_count}`;
        doc.text(line, margin, 76);
      }

      let cursorY = 92;

      // Table first — the chart image is appended below it so long tables
      // aren't pushed off multiple pages before the reader sees any numbers.
      autoTable(doc, {
        startY: cursorY,
...
const LEDGER_COLUMNS: ExportColumn[] = [
  ...
  { key: 'invoice_no', label: 'Purchase Invoice No' },
...
// 3D bar shapes for the two series. Debit (money owed on new invoices)
// uses the red/accent tone; Credit (payments made) uses green, matching
// Sales Report's convention (own-side flow = accent, settlement = green).
const debitBarShape  = make3DBarShape('url(#pDebitFrontGradient)', '#fca5a5', '#7f1d1d');
const creditBarShape = make3DBarShape('url(#pCreditFrontGradient)', '#86efac', '#14532d');

export default function PurchaseReportMaster() {
  const [rows, setRows]         = useState<PurchaseLedgerRow[]>([]);
  const [summary, setSummary]   = useState<PurchaseReportSummary | null>(null);
  const [trend, setTrend]       = useState<PurchaseTrendPoint[]>([]);
  ...
  const filters = { search, supplier, from: fromDate, to: toDate };

  const loadSuppliers = async () => {
    try { setSupplierOptions(await fetchPurchaseReportSuppliers()); }
    catch (e: any) { pushToast('error', 'Supplier List Failed', e?.message); }
  };

  const loadSummary = async () => {
    try { setSummary(await fetchPurchaseReportSummary(filters)); }
    catch (e: any) { pushToast('error', 'Summary Load Failed', e?.message); }
  };

  const loadTrend = async () => {
    setChartLoading(true);
    try { setTrend(await fetchPurchaseReportTrend({ supplier, from: fromDate, to: toDate })); }
  ...
  const loadLedger = async () => {
    setLoading(true);
    try {
      const res = await fetchPurchaseLedger({ ...filters, page, limit: pageSize });
  ...
  useEffect(() => { loadSummary(); loadTrend(); }, [supplier, fromDate, toDate]);
  useEffect(() => { loadLedger(); }, [search, supplier, fromDate, toDate, page, pageSize]);
  useEffect(() => { setPage(1); }, [search, supplier, fromDate, toDate]);
  ...
  const getAllRowsForExport = async () => {
    const res = await fetchPurchaseLedgerAll(filters);
    return res.data;
  };

  const exportFilename = `purchase-report${supplier ? `-${supplier.replace(/\s+/g, '_')}` : ''}`;
    ...
    <>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        @keyframes prToastIn { from { opacity:0; transform:translateX(20px); } to { opacity:1; transform:translateX(0); } }
        @keyframes spin { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }
        .pr-wrap { font-family:'DM Sans',sans-serif; font-size:14px; color:#1e293b; }
        .pr-toolbar { display:flex; flex-wrap:wrap; align-items:center; gap:8px; margin-bottom:12px; }
        .pr-search-wrap { position:relative; flex:1; min-width:180px; max-width:300px; }
        .pr-search-wrap svg { position:absolute; left:10px; top:50%; transform:translateY(-50%); color:#94a3b8; }
        .pr-search { width:100%; padding:8px 12px 8px 34px; border:1px solid #cbd5e1; border-radius:8px; font-size:13px; font-family:'DM Sans',sans-serif; background:#fff; color:#1e293b; outline:none; }
        .pr-search:focus { border-color:${ACCENT}; }
        .pr-filter-sel, .pr-date { border:1px solid #cbd5e1; border-radius:8px; padding:8px 10px; font-size:13px; font-family:'DM Sans',sans-serif; background:#fff; color:#374151; cursor:pointer; outline:none; }
        .pr-filter-sel { max-width:220px; }
        .pr-page-size { display:flex; align-items:center; gap:6px; font-size:13px; color:#64748b; margin-left:auto; }
        .pr-page-size select { border:1px solid #cbd5e1; border-radius:6px; padding:5px 8px; font-size:13px; font-family:'DM Sans',sans-serif; background:#fff; cursor:pointer; outline:none; }
        .pr-card { background:#fff; border-radius:12px; border:1px solid #e2e8f0; overflow:hidden; box-shadow:0 1px 6px rgba(0,0,0,0.07); }
        .pr-chart-card { background:#fff; border-radius:12px; border:1px solid #e2e8f0; padding:16px; margin-top:18px; box-shadow:0 1px 6px rgba(0,0,0,0.07); }
        .pr-table-wrap { overflow-x:auto; scrollbar-width:thin; scrollbar-color:${ACCENT} #f1f5f9; }
        .pr-table-wrap::-webkit-scrollbar { height: 8px; }
        .pr-table-wrap::-webkit-scrollbar-track { background: #f1f5f9; }
        .pr-table-wrap::-webkit-scrollbar-thumb { background: ${ACCENT}; border-radius: 8px; }
        .pr-table { width:100%; border-collapse:collapse; font-size:13px; font-family:'DM Sans',sans-serif; min-width:820px; }
        .pr-table thead tr { background:${ACCENT}; }
        .pr-table th { padding:11px 12px; color:#fff; font-weight:600; text-align:left; white-space:nowrap; font-size:12px; }
        .pr-table th.th-r { text-align:right; }
        .pr-table tbody tr:nth-child(odd)  td { background:#fff; }
        .pr-table tbody tr:nth-child(even) td { background:#f8fafc; }
        .pr-table tbody tr:hover td { filter:brightness(0.97); }
        .pr-table td { padding:10px 12px; color:#374151; font-size:12px; white-space:nowrap; }
        .pr-table td.td-r { text-align:right; }
        .pr-empty { text-align:center; padding:40px 16px; color:#94a3b8; font-size:13px; }
        .pr-pagination { display:flex; align-items:center; justify-content:space-between; padding:10px 12px; border-top:1px solid #f1f5f9; background:#f8fafc; font-size:12px; color:#64748b; flex-wrap:wrap; gap:8px; }
        .pr-pag-btns { display:flex; gap:4px; align-items:center; flex-wrap:wrap; }
        .pr-pag-btn { padding:4px 10px; border:1px solid #cbd5e1; border-radius:6px; background:#fff; cursor:pointer; font-size:12px; min-width:30px; height:30px; display:flex; align-items:center; justify-content:center; }
        .pr-pag-btn:disabled { border-color:#e2e8f0; background:#f1f5f9; color:#94a3b8; cursor:not-allowed; }
      `}</style>
      ...
      <div className="pr-wrap">
        ...
        <div style={{ marginBottom: 16 }}>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>Purchase Report</h1>
          <p style={{ margin: '3px 0 0', fontSize: 13, color: '#64748b' }}>
            Supplier-wise ledger of Purchase Invoices (Fabric + Yarn, Debit) and Payments Made (Credit), with a running balance.
          </p>
        </div>
        ...
        {summary && (
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 18 }}>
            ...
            <SummaryCard label="Total Credit" value={inr(summary.total_credit)} sub="Payments made" color="#16a34a" icon={<ArrowDownToLine size={18} />} />
            <SummaryCard label="Net Balance"  value={inr(summary.net_balance)}  sub="Debit − Credit" color={ACCENT} icon={<Wallet size={18} />} />
            <SummaryCard label="Suppliers"    value={String(summary.supplier_count)} color="#7c3aed" icon={<Truck size={18} />} />
          </div>
        )}
        ...
        <div className="pr-toolbar">
          <div className="pr-search-wrap">
            <Search size={14} />
            <input className="pr-search" placeholder="Search invoice no, supplier, bill to…" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>

          <select className="pr-filter-sel" value={supplier} onChange={(e) => setSupplier(e.target.value)} title="Filter by supplier">
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#64748b' }}>
            <Calendar size={13} />
            <input type="date" className="pr-date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} />
            <span>to</span>
            <input type="date" className="pr-date" value={toDate} onChange={(e) => setToDate(e.target.value)} />
          </span>
        ...
        <div className="pr-card">
          <div className="pr-table-wrap">
            <table className="pr-table">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={8} className="pr-empty"><Loader2 size={22} style={{ animation: 'spin 1s linear infinite', display: 'inline-block' }} /></td></tr>
                ) : rows.length === 0 ? (
                  <tr><td colSpan={8} className="pr-empty">
                    {search || supplier || fromDate || toDate ? 'No records match your filters.' : 'No purchase ledger data yet.'}
                  </td></tr>
                ) : rows.map((r, i) => (
                  <tr key={`${r.invoice_no}-${r.purchase_date}-${i}`}>
          ...
          {!loading && total > 0 && (
            <div className="pr-pagination">
              ...
              <div className="pr-pag-btns">
                <button className="pr-pag-btn" onClick={() => goTo(1)} disabled={page === 1}>«</button>
                <button className="pr-pag-btn" onClick={() => goTo(page - 1)} disabled={page === 1}>‹</button>
                <button className="pr-pag-btn" onClick={() => goTo(page + 1)} disabled={page === totalPages}>›</button>
                <button className="pr-pag-btn" onClick={() => goTo(totalPages)} disabled={page === totalPages}>»</button>
              </div>
            </div>
          )}
        </div>

        {/* TREND CHART — responsive, pseudo-3D bars for monthly Debit vs
            Credit, with a cumulative Balance line. */}
        <div className="pr-chart-card" ref={chartRef}>
          ...
          {chartLoading ? (
            ...
            <div className="pr-empty">No data for the selected filters yet.</div>
          ) : (
            <ResponsiveContainer width="100%" height={340}>
              <ComposedChart data={trend} margin={{ top: 36, right: 16, left: 0, bottom: 0 }} barGap={6} barCategoryGap="28%">
                <defs>
                  <linearGradient id="pDebitFrontGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#f87171" />
                    <stop offset="100%" stopColor="#b91c1c" />
                  </linearGradient>
                  <linearGradient id="pCreditFrontGradient" x1="0" y1="0" x2="0" y2="1">
