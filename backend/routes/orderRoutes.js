...
// ======================================================
// GET ALL  –  /api/customer-orders
// ======================================================
// Role logic:
//   admin / employee  → see all order_bookings
//   client / customer → filter by customer_name (from JWT)
//                       OR customer_id (from JWT or query param)
//                       OR customer_name query param (sent by ClientOrders.tsx)
router.get('/', auth, async (req, res) => {
  try {
    let rows;

    if (req.user.role === 'admin' || req.user.role === 'employee') {
      // ── Staff: return everything ──────────────────────────────────────────
      const [data] = await db.query(
        `SELECT * FROM order_bookings ORDER BY id DESC`
      );
      rows = data;

    } else {
      // ── Client portal user ────────────────────────────────────────────────
      // Priority 1: customer_id (most precise)
      const customerId   = req.user.customer_id   || req.query.customer_id   || null;
      // Priority 2: customer_name — sent by ClientOrders.tsx as ?customer_name=
      const customerName = req.user.name          || req.query.customer_name || null;

      if (customerId) {
        // Filter by numeric customer_id — exact match
        const [data] = await db.query(
          `SELECT * FROM order_bookings
           WHERE customer_id = ?
           ORDER BY id DESC`,
          [customerId]
        );
        ...
        // Filter by customer_name — exact match first, LIKE fallback
        ...
        if (exact.length > 0) {
          ...
          // Fallback: partial match in case of minor name differences
        ...
        // No identifier at all — return empty rather than crash
        console.warn('[GET /customer-orders] No customer_id or customer_name in JWT or query');
        return res.json([]);
      }
    }
